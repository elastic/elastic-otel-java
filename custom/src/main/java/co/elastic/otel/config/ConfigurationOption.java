...
/**
 * Class definition is still in progress, this is a partial reconstruction
 * of the stagemonitor ConfigurationOption, to be completed as we fill out
 * the configurations already defined in the original agent and decide what
 * we need and what can be implemented less abstractly
 * @param <T>
 */
...
//        private ConfigurationRegistry configuration;
  ...
  protected ConfigurationOption(boolean dynamic, boolean sensitive, String key, String label, String description,
  ...
  public static class ConfigurationOptionBuilder<T> {
    ...
    public ConfigurationOption<T> build() {
      return new ConfigurationOption<T>(dynamic, sensitive, key, label, description, defaultValue, configurationCategory,
          valueConverter, valueType, Arrays.asList(tags), required, changeListeners, validators,
          Arrays.asList(aliasKeys), validOptions);
    }

    public ConfigurationOption<T> buildNotEnabled() {
      return new ConfigurationOption<T>(false, false, key, null, null, null, null, null, null, null, false, new ArrayList<>(), new ArrayList<>(), new ArrayList<>(), null);
    }
  }

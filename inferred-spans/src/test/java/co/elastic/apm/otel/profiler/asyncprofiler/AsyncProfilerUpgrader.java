...
public class AsyncProfilerUpgrader {
  ...
  void updateAsyncProfilerBinaries() throws Exception {
    ...
    Path thisOsLib =
        getBinariesResourceDir()
            .resolve(
                co.elastic.apm.agent.profiler.asyncprofiler.AsyncProfiler.getLibraryFileName()
                    + ".so");
  ...
  private void downloadAndReplaceBinary(
    ...
    if (artifactNamePattern == null) {
      System.out.println(
          ghAssetName + " is not within the list of used artifacts, skipping download");
      return;
    }
    ...
    assertThat(localBinaryPath)
        ...
        .isNotNull();
    System.out.println(
        String.format("Binary file for %s was extracted into %s", ghAssetName, localBinaryPath));
  ...
  private void replaceBinary(Path downloadedArtifact, String artifactName) throws Exception {
    ...
    if (!Files.exists(binaryResourcePath)) {
      throw new IllegalStateException(
          String.format("Expected binary file does not exist: %s", binaryResourcePath.toString()));
    }
    System.out.println(

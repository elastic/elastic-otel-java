package co.elastic.apm.otel.profiler.config;
...
public abstract class WildcardMatcher {
  ...
  public static WildcardMatcher valueOf(final String wildcardString) {
    ...
    if (split.length == 1) {
      return new SimpleWildcardMatcher(split[0], matcher.startsWith(WILDCARD),
          matcher.endsWith(WILDCARD), ignoreCase);
    }
   ...
   */
  @Nullable
  public static WildcardMatcher anyMatch(List<WildcardMatcher> matchers, CharSequence firstPart,
      @Nullable CharSequence secondPart) {
   ...
   * Thx to Zach Vorhies
   */
  public static int indexOfIgnoreCase(final CharSequence haystack1, final CharSequence haystack2,
      final String needle, final boolean ignoreCase, final int start, final int end) {
    if (start < 0) {
      return -1;
    }
    ...
    for (int i = start; i < end; i++) {
      ...
      while (ii < totalHaystackLength && j < needleLength) {
        char c =
            ignoreCase ? Character.toLowerCase(charAt(ii, haystack1, haystack2, haystack1Length))
  ...
  static class CompoundWildcardMatcher extends WildcardMatcher {
    ...
    CompoundWildcardMatcher(String wildcardString, String matcher,
        List<SimpleWildcardMatcher> wildcardMatchers) {
  ...
  static class SimpleWildcardMatcher extends WildcardMatcher {
    ...
    SimpleWildcardMatcher(String matcher, boolean wildcardAtBeginning, boolean wildcardAtEnd,
        boolean ignoreCase) {
      this.matcher = matcher;
      ...
      this.stringRepresentation = new StringBuilder(
          matcher.length() + CASE_SENSITIVE_PREFIX.length() + WILDCARD.length() + WILDCARD.length())
    ...
    int indexOf(CharSequence firstPart, @Nullable CharSequence secondPart, int offset) {
      ...
      if (wildcardAtEnd && wildcardAtBeginning) {
        ...
        return indexOfIgnoreCase(firstPart, secondPart, matcher, ignoreCase,
            totalLength - matcher.length(), totalLength);

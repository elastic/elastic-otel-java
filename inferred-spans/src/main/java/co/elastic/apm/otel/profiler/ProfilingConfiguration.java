...
public class ProfilingConfiguration extends ConfigurationOptionProvider {
  ...
  private final ConfigurationOption<Boolean> profilingEnabled =
      ConfigurationOption.<Boolean>booleanOption()
  ...
  private final ConfigurationOption<Integer> asyncProfilerSafeMode =
      ConfigurationOption.<Integer>integerOption()
          .key("async_profiler_safe_mode")
          .configurationCategory(PROFILING_CATEGORY)
          .dynamic(false)
          .description(
              "Can be used for analysis: the Async Profiler's area that deals with recovering stack trace frames \n"
  ...
  private final ConfigurationOption<Boolean> postProcessingEnabled =
      ConfigurationOption.<Boolean>booleanOption()
          ...
          .dynamic(true)
          .description(
              "Can be used to test the effect of the async-profiler in isolation from the agent's post-processing.")
          .tags("added[1.18.0]", "internal")
          .buildWithDefault(true);

  private final ConfigurationOption<TimeDuration> samplingInterval =
      TimeDurationValueConverter.durationOption("ms")
  ...
  private final ConfigurationOption<TimeDuration> inferredSpansMinDuration =
      TimeDurationValueConverter.durationOption("ms")
  ...
  private final ConfigurationOption<List<WildcardMatcher>> includedClasses =
      ConfigurationOption.builder(
              new ListValueConverter<>(new WildcardMatcherValueConverter()), List.class)
  ...
  private final ConfigurationOption<List<WildcardMatcher>> excludedClasses =
      ConfigurationOption.builder(
              new ListValueConverter<>(new WildcardMatcherValueConverter()), List.class)
  ...
  private final ConfigurationOption<TimeDuration> profilerInterval =
      TimeDurationValueConverter.durationOption("s")
  ...
  private final ConfigurationOption<TimeDuration> profilingDuration =
      TimeDurationValueConverter.durationOption("s")
  ...
  private final ConfigurationOption<String> profilerLibDirectory =
      ConfigurationOption.<String>stringOption()
          .key("profiling_inferred_spans_lib_directory")
          .description(
              "Profiling requires that the https://github.com/jvm-profiling-tools/async-profiler[async-profiler] shared library "
  ...
  public String getProfilerLibDirectory() {
    return profilerLibDirectory.isDefault()
        ? System.getProperty("java.io.tmpdir")
        : profilerLibDirectory.get();
  }

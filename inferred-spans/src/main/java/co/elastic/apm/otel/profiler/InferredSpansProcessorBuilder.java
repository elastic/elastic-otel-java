...
public class InferredSpansProcessorBuilder {
  ...
  private List<WildcardMatcher> excludedClasses = Arrays.asList(
  ...
  public InferredSpansProcessor build() {
    InferredSpansConfiguration config = new InferredSpansConfiguration(
    ...
    return new InferredSpansProcessor(config, clock, startScheduledProfiling, activationEventsFile,
        jfrFile);
  }

  /**
   * By default, async profiler prints warning messages about missing JVM symbols to standard output.
   * Set this option to {@code true} to suppress such messages
   */
   ...
   * If you want to have more profiling coverage, try decreasing {@link #profilerInterval(Duration)}.
   */

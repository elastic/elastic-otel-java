...
public class JavaExecutable {
    ...
    private static boolean probeListeningDebugger(int port) {
        ...
        try {
            Process process = new ProcessBuilder()
                    .command(JavaExecutable.getBinaryPath().toString(), jvmDebugArgument("localhost", port), "-version")
                    .start();
    ...
    public static String jvmDebugArgument(String host, int port) {
        return String.format("-agentlib:jdwp=transport=dt_socket,server=n,address=%s:%d,suspend=y", host, port);
    }
    ...
    public static boolean isDebuggingAndListeningDebuggerStarted(int port, String description){
        ...
        if(probeListeningDebugger(port)){
            ...
            log.info("listening debugger not detected on port {}, remote debugging in the {} container is not enabled", port, description);
            return false;
        }

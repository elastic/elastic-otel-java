...
https://opentelemetry.io/docs/zero-code/java/agent/configuration/[OpenTelemetry documentation],
including:
...
of configuration options that are only available in the {distro_name}:

* Additional `OTEL_` options that Elastic plans to contribute upstream to the https://opentelemetry.io/docs/languages/{language_lc}[OpenTelemetry SDK for {language}], but are not yet available in the OpenTelemetry SDK for {language}.
* `ELASTIC_OTEL_` options that are specific to Elastic and will always live in the {distro_name} (in other words, it will _not_ be added upstream).

////
TO DO:
List config options instead of linking to the README
////
Read more about configuration options that are only available in the distro in https://github.com/elastic/elastic-otel-java?tab=readme-ov-file#features[the elastic/elastic-otel-java README].

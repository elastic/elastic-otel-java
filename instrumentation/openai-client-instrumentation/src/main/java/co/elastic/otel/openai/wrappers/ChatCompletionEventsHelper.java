...
public class ChatCompletionEventsHelper {
    ...
    private static final Logger EV_LOGGER = GlobalOpenTelemetry.get().getLogsBridge().get(Constants.INSTRUMENTATION_NAME);

    private static final AttributeKey<String> EVENT_NAME_KEY = AttributeKey.stringKey("event.name");

    public static void emitPromptLogEvents(ChatCompletionCreateParams request, InstrumentationSettings settings) {
        ...
        for (ChatCompletionMessageParam msg : request.messages()) {
            ...
            if (msg.isChatCompletionSystemMessageParam()) {
                ...
                ChatCompletionAssistantMessageParam assistantMsg = msg.asChatCompletionAssistantMessageParam();
                eventType = "gen_ai.assistant.message";
                if (settings.captureMessageContent) {
                    assistantMsg.content().ifPresent(content -> putIfNotEmpty(bodyBuilder, "content", contentToString(content)));
                    assistantMsg.toolCalls().ifPresent(toolCalls -> {
                        List<Value<?>> toolCallsJson = toolCalls.stream()
    ...
    public static void emitCompletionLogEvents(ChatCompletion completion, InstrumentationSettings settings) {
        if (!settings.emitEvents) {
            return;
        }
        for (ChatCompletion.Choice choice : completion.choices()) {
            ...
            if (settings.captureMessageContent) {
                choiceMsg.content().ifPresent(content -> message.put("content",
                        Value.of(content)
                ));
                choiceMsg.toolCalls().ifPresent(toolCalls -> {
                    message.put("tool_calls", Value.of(toolCalls.stream()
                            .map(ChatCompletionEventsHelper::buildToolCallEventObject)
                            .collect(Collectors.toList())));
                });
            }
            emitCompletionLogEvent(choice.index(), choice.finishReason().toString(), Value.of(message), settings, null);
        }
    }

    public static void emitCompletionLogEvent(long index, String finishReason, Value<?> eventMessageObject, InstrumentationSettings settings, Context contextOverride) {
        if (!settings.emitEvents) {
            return;
        }
        LogRecordBuilder builder = newEvent("gen_ai.choice")
                .setBody(new MapValueBuilder()
    ...
    private static LogRecordBuilder newEvent(String name) {
        return EV_LOGGER.logRecordBuilder()
    ...
    private static Value<?> buildFunctionEventObject(ChatCompletionMessageToolCall.Function function) {
        Map<String, Value<?>> result = new HashMap<>();

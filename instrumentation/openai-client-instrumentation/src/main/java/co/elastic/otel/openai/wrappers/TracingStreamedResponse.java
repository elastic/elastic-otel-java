...
public class TracingStreamedResponse implements StreamResponse<ChatCompletionChunk> {
    ...
    public TracingStreamedResponse(StreamResponse<ChatCompletionChunk> delegate, Context spanContext, InstrumentedChatCompletionService.RequestHolder requestHolder) {
    ...
    private synchronized void endSpan() {
        ...
        List<String> finishReasons = choiceFinishReasons.entrySet().stream()
        ...
        InstrumentedChatCompletionService.ChatCompletionResult result = new InstrumentedChatCompletionService.ChatCompletionResult(model, responseId, finishReasons, inputTokens, completionTokens);
    ...
    private class TracingSpliterator implements Spliterator<ChatCompletionChunk> {
        ...
        public boolean tryAdvance(Consumer<? super ChatCompletionChunk> action) {
            boolean chunkReceived = delegateSpliterator.tryAdvance(chunk -> {
